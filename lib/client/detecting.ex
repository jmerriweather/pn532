defmodule PN532.Client.Detecting do
  ...
  def detecting(:info, {:circuits_uart, com_port, <<0xD5, 0x61, 0, _rest::bitstring>>}, data = %{current_cards: current_cards, handler: handler, connection: connection, connection_options: connection_options}) do
    ...
    {:noreply, %{connection: new_connection, connection_options: new_connection_options}} =
      if current_cards != nil do
        apply(handler, :handle_event, [:cards_lost, current_cards, %{connection: connection, connection_options: connection_options}])
      end

    {:keep_state, %{data | current_cards: nil, poll_number: 255, connection: new_connection, connection_options: new_connection_options}, [{:state_timeout, 100, :poll_for_cards}]}
  end
    ...
    with {:ok, cards} <- detected do
      ...
      {:noreply, %{connection: new_connection, connection_options: new_connection_options}} =
        if current_cards !== cards do
          apply(handler, :handle_event, [:cards_detected, cards, %{connection: connection, connection_options: connection_options}])
        end
      {:keep_state, %{data | current_cards: cards, poll_number: 7, connection: new_connection, connection_options: new_connection_options}, [{:state_timeout, 100, :poll_for_cards}]}
    else
      _ ->
